...
def is_ip_range_or_subnet(ip_str):
    return bool(re.match(r'^[\d\.\-\/\s]+$', ip_str)) and ('-' in ip_str or '/' in ip_str)
...
# [적용] Request URL 속성 분기 헬퍼 (Simple / Regex / Advanced)
def generate_a_url_xml(obj_name, url_val, is_regex=False):
    url_val_str = str(url_val)
    
    # 1. 정규식(Regex) Match 처리
    ...
    # 2. Advanced Match (포트 포함: host:port/path)
...
def format_address_group(addrs_list, group_dict):
    ...
    for addr in addrs_list:
        if addr in group_dict and group_dict[addr]:
            ...
            if len(items) == 1:
                res.append(f"<div style='margin-bottom:3px;'><span style='font-weight:bold; color:#2c3e50;'>{escape_xml(addr)}</span> <span style='font-size:9px; color:#7f8c8d; background:#ecf0f1; padding:2px 4px; border-radius:3px;'>{escape_xml(items[0])}</span></div>")
            else:
...
def get_sym_obj_html(obj_name, group_map):
    ...
    if obj_name in group_map:
        items = flatten_sym_group(obj_name, group_map)
        items = list(dict.fromkeys(items))
        if len(items) == 1:
            return f"<div style='margin-bottom:3px;'><span style='font-weight:bold; color:#2980b9;'>{escape_xml(obj_name)}</span> <span style='font-size:9px; color:#7f8c8d; background:#eaf2f8; padding:2px 4px; border-radius:3px;'>{escape_xml(items[0])}</span></div>"
        else:
            display_items = items[:100]
            items_text = "\n".join([escape_xml(i) for i in display_items])
            if len(items) > 100: items_text += f"\n\n... 외 {len(items)-100}개 항목 생략됨"
            return f"<details><summary style='cursor:pointer; font-weight:bold; color:#2980b9;'>{escape_xml(obj_name)} <span style='font-size:10px; color:#7f8c8d;'>({len(items)})</span></summary><div style='margin:4px 0 6px 0; padding: 5px; background: #eaf2f8; border: 1px solid #ddd; max-height: 120px; overflow-y: auto; font-size:10px; color:#333; white-space: pre-wrap; word-break: break-all;'>{items_text}</div></details>"
    else:
        return f"<div style='margin-bottom:3px; font-weight:bold; color:#2980b9;'>{escape_xml(obj_name)}</div>"
...
def get_obj_details(xml_str, obj_type):
    m_name = re.search(r'(?:name|n)="([^"]+)"', xml_str)
    ...
    if obj_type == "IP 단일/범위 객체":
        ...
        if m_url: content = m_url.group(1)
            ...
            if m_p: parts.append("Path: " + m_p.group(1))
            content = " / ".join(parts)
        else:
...
def extract_policy_to_json(input_data, skip_auto_update=False):
    ...
    for outer_match in target_outer_regex.finditer(input_data):
        for match in target_regex.finditer(outer_match.group(1)):
            ...
            ips = [line.strip() for line in match.group(2).split('\n') if line.strip() and not line.startswith('<') and not line.startswith('define') and not line.startswith('end')]
            group_dict[name] = [re.sub(r'\s+all', '', ip, flags=re.IGNORECASE).strip() for ip in ips]
    ...
    for outer_match in webcat_outer_regex.finditer(input_data):
        for match in webcat_regex.finditer(outer_match.group(1)):
            ...
            if skip_auto_update and name == "Auto_Update_Blacklist_Deny":
                continue
            urls = [clean_url_trailing_tuple(line) for line in match.group(2).split('\n') if line.strip() and not line.startswith('<') and not line.startswith('define') and not line.startswith('end')]
            group_dict[name] = urls
    ...
    for match in policy_regex.finditer(input_data):
        ...
        for d_block in dir_blocks[1:]:
            ...
            for line in d_block.splitlines()[1:]:
                ...
                if name_match:
                    if direction == 'inbound': inbound_names.append(name_match.group(1).strip())
                    elif direction == 'outbound': outbound_names.append(name_match.group(1).strip())
                else: break
        
        user_cat_m = re.search(r'userDefinedCategory\s+name=(.*)', p_body, re.IGNORECASE)
        if user_cat_m: outbound_names.append(user_cat_m.group(1).strip())
        web_cat_ids = re.findall(r'web-category-id\s+value=(.*)', p_body, re.IGNORECASE)
        for w in web_cat_ids: outbound_names.append(w.strip())
        
        # [픽스] 2번 소만사 JSON에 webUrlList 포함되도록 추가
        web_url_list_m = re.search(r'webUrlList:\s*(.*)', p_body, re.IGNORECASE)
        if web_url_list_m:
            raw_urls = web_url_list_m.group(1).strip()
            if raw_urls:
                urls = [clean_url_trailing_tuple(u) for u in raw_urls.split(',') if u.strip()]
...
<body>
    <div class="top-section">
        ...
        <div class="tabs-wrapper">
            {tabs_html}
            <div id="paginationControls" style="display: flex; align-items: center;"></div>
...
"""
    ...
    for idx, som in enumerate(json_data):
        ...
        if is_pass:
            current_group_id += 1
            html_content += f"""
            <tbody class="paginate-item pass-divider" id="group-header-{current_group_id}" onclick="toggleSection({current_group_id}, event)" title="클릭하여 하위 정책 접기/펼치기">
            ...
            html_content += f"""
            <tbody class="paginate-item group-item group-item-{current_group_id}{hidden_cls}">
                <tr class="{row_cls}">
...
def clean_flat(flat_list):
    return set([str(x).strip() for x in flat_list if str(x).strip().upper() not in ("ANY", "NONE", "")])
        ...
        <table id="compTable">
            <thead>
                <tr>
                    ...
                    <th style="width: 15%;">정책명 / Track (Layer)</th>
                    <th style="width: 5%;">상태</th>
                    <th style="width: 12%;">출발지 (Source)</th>
                    <th style="width: 12%;">목적지 (Destination)</th>
                    <th style="width: 20%;">조건 (Rules / Service)</th>
                    <th style="width: 6%;">액션</th>
                    <th style="width: 10%;">차단 메시지</th>
                </tr>
            </thead>
"""
    current_group_id = 0
    for idx, som in enumerate(somansa_json):
        ...
        if is_pass:
            current_group_id += 1
            html_content += f"""
            <tbody id="group-header-{current_group_id}" class="paginate-item pass-divider">
                <tr>
            ...
            diff_cond = "diff-cell" if str(som.get('policyHttpRules', '')).strip() != str(sym.get('Service', '')).strip() else ""
            ...
            html_content += f"""
            <tbody class="paginate-item policy-group">
...
# --- 핵심 파싱 로직 ---
def convert_somansa_to_symantec(input_data, mode, optimize_choice, internal_networks, skip_auto_update=False):
    # 단일 URL 중복 방지 전역 맵 선언: URL -> Object Name
    global_single_url_map = {}
    # 다중 URL 중복 방지 전역 맵 선언: tuple(urls) -> (node_name, cat_list_name)
    global_url_list_map = {}
    ...
    for outer_match in webcat_outer_regex.finditer(input_data):
        for match in webcat_regex.finditer(outer_match.group(1)):
            ...
            if valid_urls:
                valid_urls = [clean_url_trailing_tuple(u) for u in valid_urls]
                
                if len(valid_urls) == 1:
                    url_val = valid_urls[0]
                    if url_val in global_single_url_map:
                        cat_node_name = global_single_url_map[url_val]
                    else:
                        cat_node_name = get_unique_node_name(final_cat_name)
                        global_single_url_map[url_val] = cat_node_name
                        req_url_objects.append(generate_a_url_xml(cat_node_name, url_val))
                    
                    webcat_name_map[name] = cat_node_name
                    symantec_group_map[cat_node_name] = [url_val]
                else:
                    url_tuple = tuple(valid_urls)
                    if url_tuple in global_url_list_map:
                        cat_node_name, _ = global_url_list_map[url_tuple]
                        webcat_name_map[name] = cat_node_name
                    else:
                        url_string = '&#10;'.join([escape_xml(u) for u in valid_urls])
    ...
    for t_name, raw_ips in raw_targets.items():
        ...
        for line in raw_ips:
            ...
            if not line or line.startswith('<') or line.startswith('define') or line.startswith('end'):
                continue
        ...
        if "User_IP_Exception_ServerFarm" in t_name:
            ...
            if domain_urls:
                base_name = t_name if not priv_ips and not pub_ips else f"{t_name}_URL"
                url_obj_name = format_obj_name(base_name, 'd_')
                
                if len(domain_urls) == 1:
                    url_val = domain_urls[0]
                    if url_val in global_single_url_map:
                        url_obj_name = global_single_url_map[url_val]
                    else:
                        url_obj_name = get_unique_node_name(url_obj_name)
                        global_single_url_map[url_val] = url_obj_name
                        req_url_objects.append(generate_a_url_xml(url_obj_name, url_val))
                        
                    symantec_group_map[url_obj_name] = [url_val]
                    dest_objs.append(url_obj_name)
                else:
                    url_tuple = tuple(domain_urls)
                    if url_tuple in global_url_list_map:
                        _, cat_list_name = global_url_list_map[url_tuple]
                        dest_objs.append(cat_list_name)
                    else:
                        url_string = '&#10;'.join([escape_xml(ip) for ip in domain_urls])
                        
                        cat_node_name = get_unique_node_name(url_obj_name)
                        vpm_cat_nodes.append(f'<node n="{escape_xml(cat_node_name)}" u-l="{url_string}"/>')
                        
                        cat_tuple_group = (cat_node_name,)
                        if cat_tuple_group not in category_list_map:
                            if len(cat_tuple_group) == 1:
                                cat_list_name = cat_tuple_group[0]
                            else:
                            ...
                            cat_xml = f'<categorylist4 name="{escape_xml(cat_list_name)}" typ="r">\n<sel>\n{i_tags}\n</sel>\n<excl/>\n<sel-p/>\n</categorylist4>'
                            category_list_objects.append(cat_xml)
                            symantec_group_map[cat_list_name] = list(cat_tuple_group)
                        else:
                            cat_list_name = category_list_map[cat_tuple_group]
    ...
    def parse_policy(is_tls, block):
        for match in policy_regex.finditer(block):
            ...
            if active_match and expire_match:
                ...
                ti_name = format_obj_name(f"Time_{active_date}_{expire_date}")
                time_obj_xml = f'<time name="{escape_xml(ti_name)}" UTC="false" time="0000..2359" absoluterange="{active_date}..{expire_date}" single="true" value="{escape_xml(ti_name)}"/>'
                ...
                def build_ast(node):
                    if "condition" in node and "rules" in node:
                        ...
                        if rid == "http:method":
                            ...
                            # [적용] HTTP URL 규칙에 포함된 단일 객체 Deduplication 및 <a-url> 생성
                            is_equal = (op == "equal")
                            hash_key = f"{val_str}_{op}"
            ...
            if is_tls:
                ...
                web_cat_ids = re.findall(r'web-category-id\s+value=(.*)', p_body, re.IGNORECASE)
                web_cat_ids = [c.strip() for c in web_cat_ids if c.strip() and c.strip().upper() != "ANY"]
                user_cat_name = user_cat_match.group(1).strip() if user_cat_match else ""
                ...
                if web_cat_ids:
                    if web_url_list_match := re.search(r'webUrlList:\s*(.*)', p_body, re.IGNORECASE):
                        web_url_list = web_url_list_match.group(1).strip()
                        if web_url_list:
                            urls = [clean_url_trailing_tuple(u) for u in web_url_list.split(',') if u.strip()]
                            base_cat_name = webcat_name_map.get(web_cat_ids[0], format_obj_name(web_cat_ids[0]))
                            ...
                            if len(urls) == 1:
                                ...
                                if url_val in global_single_url_map:
                                    ...
                                    url_obj_name = format_obj_name(f"{base_cat_name}_URL")
                                    url_obj_name = get_unique_node_name(url_obj_name)
                                    global_single_url_map[url_val] = url_obj_name
                                    req_url_objects.append(generate_a_url_xml(url_obj_name, url_val))
                                    
                                symantec_group_map[url_obj_name] = [url_val]
                                mapped_outbounds.append(url_obj_name)
                                web_cat_ids.pop(0)
                            elif len(urls) > 1:
                                url_tuple = tuple(urls)
                                if url_tuple in global_url_list_map:
                                    _, cat_list_name = global_url_list_map[url_tuple]
                                    mapped_outbounds.append(cat_list_name)
                                    web_cat_ids.pop(0)
                                else:
                                    url_string = '&#10;'.join([escape_xml(u) for u in urls])
                                    
                                    cat_node_name = get_unique_node_name(base_cat_name)
                                    vpm_cat_nodes.append(f'<node n="{escape_xml(cat_node_name)}" u-l="{url_string}"/>')
                                    
                                    cat_tuple_group = (cat_node_name,)
                                    if cat_tuple_group not in category_list_map:
                                        cat_list_name = format_obj_name(f"{base_cat_name}_{len(category_list_map) + 1}")
                                        category_list_map[cat_tuple_group] = cat_list_name
                                        i_tags = f'<i>{escape_xml(cat_node_name)}</i>'
                                        cat_xml = f'<categorylist4 name="{escape_xml(cat_list_name)}" typ="r">\n<sel>\n{i_tags}\n</sel>\n<excl/>\n<sel-p/>\n</categorylist4>'
                                        category_list_objects.append(cat_xml)
...
<colItem col="{col_offset+3}" id="co" {get_col_attrs(comment, 'co')}/>
    ...
    if optimize_choice == 'n':
        ...
        if mode == "legacy":
            for l in legacy_tls_layers: all_rows.extend(l["rows"])
            for l in legacy_http_layers: all_rows.extend(l["rows"])
        else:
            all_rows = tls_row_items + http_row_items + req_row_items + custom_cat_row_items
            
        for row in all_rows:
            matches = re.findall(r'name="([^"]+)"', row)
            for m in matches:
                used_names.add(m.replace("&amp;", "&").replace("&lt;", "<").replace("&gt;", ">").replace("&quot;", '"').replace("&apos;", "'"))

        while True:
            added_new = False
            for comb_xml in unique_comb_objects:
                m = re.search(r'<comb-obj name="([^"]+)"', comb_xml)
                if m and m.group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names:
                    inner_names = re.findall(r'<c-l-\d+ n="([^"]+)"', comb_xml)
                    for inner in inner_names:
                        clean_inner = inner.replace("&amp;", "&").replace("&quot;", '"')
                        if clean_inner not in used_names:
                            used_names.add(clean_inner)
                            added_new = True
        ...
        while True:
            ...
            for cat_list_xml in unique_category_list_objects:
                m = re.search(r'<categorylist4 name="([^"]+)"', cat_list_xml)
                if m and m.group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names:
        ...
        active_comb_objects = [xml for xml in unique_comb_objects if re.search(r'<comb-obj name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_category_list_objects = [xml for xml in unique_category_list_objects if re.search(r'<categorylist4 name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_ip_objects = [xml for xml in unique_ip_objects if re.search(r'name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_ip_list_objects = [xml for xml in unique_ip_list_objects if re.search(r'name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_host_port_objects = [xml for xml in unique_host_port_objects if re.search(r'name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_a_url_objects = [xml for xml in unique_a_url_objects if re.search(r'name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_req_url_objects = [xml for xml in unique_req_url_objects if re.search(r'name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_req_hdr_objects = [xml for xml in unique_req_hdr_objects if re.search(r'name="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
        active_vpm_cat_nodes = [xml for xml in unique_vpm_cat_nodes if re.search(r'<node n="([^"]+)"', xml).group(1).replace("&amp;", "&").replace("&quot;", '"') in used_names]
...
<colItem col="8" id="co" name="" type="String"/>
    ...
    output_xml += '</layers>\n</vpmapp>'
    return output_xml, exception_pages_text, removed_csv_data, exception_tracked_list, symantec_json_data
...
if __name__ == "__main__":
    ...
    # 실행 파일 기준 동일 폴더 내의 지정된 파일 자동 탐지
    ...
    print(f"{Fore.CYAN}========================================")
    mode_input = input(f"{Fore.YELLOW}👉 방식을 선택하세요 (1/2) [기본값: 1]: ").strip()
    selected_mode = "new" if mode_input == '2' else "legacy"
    ...
    # 3. 핵심 시만텍 XML 변환
    print(f"{Fore.GREEN}▶ [3/6] 시만텍 VPM XML(5번) 변환 및 최적화 진행 중 (시간이 소요될 수 있습니다)...")
    output_xml, exception_pages_text, removed_csv_data, tracked_exceptions, symantec_json_data = convert_somansa_to_symantec(raw_input, selected_mode, optimize_choice, internal_networks, skip_auto_update)
    ...
    if tracked_exceptions:
        print(f"\n{Fore.MAGENTA}========================================")
        print(f"{Fore.MAGENTA}{Style.BRIGHT} 🚨 [예외 처리 적용된 객체 목록]")
