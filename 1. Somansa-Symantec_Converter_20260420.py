    ...
    for outer_match in webcat_outer_regex.finditer(input_data):
        for match in webcat_regex.finditer(outer_match.group(1)):
            ...
            if skip_auto_update and name == "Auto_Update_Blacklist_Deny":
                continue
            urls = [clean_url_trailing_tuple(urllib.parse.unquote(line.strip())) for line in match.group(2).split('\n') if line.strip() and not line.startswith('<') and not line.startswith('define') and not line.startswith('end')]
            valid_urls = list(dict.fromkeys(urls))
            group_dict[name] = valid_urls
...
<colItem col="{col_offset+3}" id="co" {get_col_attrs(comment, 'co')}/>
    ...
    if optimize_choice == 'n':
        ...
        if mode == "legacy":
            ...
            all_rows = tls_row_items + http_row_items
        else:
...
if __name__ == "__main__":
    ...
    print(f"{Fore.CYAN}========================================")
    mode_input = input(f"{Fore.YELLOW}👉 방식을 선택하세요 (1/2/3) [기본값: 1]: ").strip()
    if mode_input == '2':
        selected_mode = "new"
    elif mode_input == '3':
    ...
    # 4. XML 및 CSV 저장 (진행과정 삭제 객체 수 통합)
    if removed_csv_data:
